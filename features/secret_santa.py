...
def add_user_to_room(user: User, command: str):
    ...
    if room_id:
        ...
        if room == 'exists':
            ...
            room_admin_id = Users.get_or_none(social=user.social.key, room_id=room_id, is_admin=True)
            if room_admin_id:
                ...
                msend.send_msg(room_admin_id.user_social_id, user.social, Message(cmng.user_adding.descr.format(user_name)))
            else:
